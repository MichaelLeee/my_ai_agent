...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app my_ai_agent db upgrade
	@echo ""
	@echo "🚀 Dev stack ready:"
	@echo "   API:      http://localhost:8080"
	@echo "   Docs:     http://localhost:8080/docs"
	@echo "   Admin:    http://localhost:8080/admin"
	@echo "   Frontend: http://localhost:3030  (run 'make dev-frontend' or 'cd frontend && bun dev')"
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		my_ai_agent user list 2>/dev/null \
		...
		docker compose -f docker-compose.dev.yml exec -T app \
			my_ai_agent user create \
				--email admin@example.com --password admin123 --superuser \
		&& echo "  ✅ Admin created. Login at http://localhost:8080/admin"; \
	fi
...
dev-frontend:
	...
	@echo ""
	@echo "✅ Frontend at http://localhost:3030  (backend must be up — 'make dev')"

# === Staging: built images, no bind mounts (production-like, local DB) ===
stage:
	docker compose -f docker-compose.yml up -d --build
	$(call _wait_for_db,docker-compose.yml)
	docker compose -f docker-compose.yml exec -T app my_ai_agent db upgrade
	@echo "✅ Staging stack at http://localhost:8080"
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app my_ai_agent db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run my_ai_agent db migrate -m "initial" || true
	cd backend && uv run my_ai_agent db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend my_ai_agent db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend my_ai_agent db upgrade

db-downgrade:
	uv run --directory backend my_ai_agent db downgrade

db-current:
	uv run --directory backend my_ai_agent db current

db-history:
	uv run --directory backend my_ai_agent db history

# === Server ===
run:
	uv run --directory backend my_ai_agent server run --reload

run-prod:
	uv run --directory backend my_ai_agent server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend my_ai_agent server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend my_ai_agent user create-admin

user-create:
	uv run --directory backend my_ai_agent user create

user-list:
	uv run --directory backend my_ai_agent user list

# === Celery ===
celery-worker:
	uv run --directory backend my_ai_agent celery worker

celery-beat:
	uv run --directory backend my_ai_agent celery beat

celery-flower:
	uv run --directory backend my_ai_agent celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/my_ai_agent"
...
help:
	@echo ""
	@echo "my_ai_agent - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run my_ai_agent rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run my_ai_agent rag-search <query> -c <collection>  Search"
	@echo "  uv run my_ai_agent rag-collections                     List collections"
	@echo "  uv run my_ai_agent rag-sources                         List sync sources"
	@echo "  uv run my_ai_agent rag-source-add                      Add sync source"
	@echo "  uv run my_ai_agent rag-source-sync <id>                Trigger sync"
	@echo ""
	@echo "Celery:"
