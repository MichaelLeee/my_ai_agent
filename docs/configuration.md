...
|----------|---------|-------------|
| `PROJECT_NAME` | `my_ai_agent` | Display name for the project |
...
| `TIMEZONE` | `Americas/Los_Angeles` | IANA timezone (e.g. `UTC`, `Europe/Warsaw`, `America/New_York`) |
...
| `POSTGRES_DB` | `my_ai_agent` | Database name |
...
| `LLM_PROVIDER` | `anthropic` | LLM provider (informational) |
...
| `LOGFIRE_SERVICE_NAME` | `my_ai_agent` | Service name in Logfire dashboard |
