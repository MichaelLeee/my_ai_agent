...
```bash
curl -X POST http://localhost:8080/api/v1/rag/sync/sources \
...
```bash
curl -X POST http://localhost:8080/api/v1/rag/sync/sources/{source_id}/trigger \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8080/api/v1/rag/sync/logs?limit=10 \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8080/api/v1/rag/sync/connectors \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X PATCH http://localhost:8080/api/v1/rag/sync/sources/{source_id} \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "sync_mode": "full",
...
```bash
curl http://localhost:8080/api/v1/rag/sync/logs?collection_name=legal&limit=5 \
  -H "Authorization: Bearer $TOKEN"
```
