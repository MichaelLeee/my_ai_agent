...
- **CLI** — `uv run my_ai_agent rag-sync` commands
...
uv run my_ai_agent cmd rag-sync --connector my_source \
...
# Trigger a sync
curl -X POST http://localhost:8080/api/v1/rag/sync/sources/{source_id}/sync \
    -H "Authorization: Bearer $TOKEN"
```
