...
Reference for `my_ai_agent` runtime configuration. The
...
|---|---|---|---|
| `PROJECT_NAME` | optional | `my_ai_agent` | Used in logs, OpenAPI title, email templates |
...
| `TIMEZONE` | optional | `Americas/Los_Angeles` | IANA TZ name (e.g. `Europe/Warsaw`) |
| `BACKEND_URL` | optional | `http://localhost:8080` | Used by frontend BFF + email link generation |
| `FRONTEND_URL` | optional | `http://localhost:3030` | Used by password-reset / magic-link emails |
...
| `BILLING_TRIAL_DAYS` | optional | `180` | Default trial length |
