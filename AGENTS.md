...
**my_ai_agent** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL
, JWT + API Key auth, Redis
, pydantic_ai (anthropic), RAG (pgvector), Next.js 15 (i18n)
...
# RAG
uv run my_ai_agent rag-ingest /path/to/file.pdf --collection docs
uv run my_ai_agent rag-search "query" --collection docs

# Sync Sources
uv run my_ai_agent cmd rag-sources
uv run my_ai_agent cmd rag-source-add
uv run my_ai_agent cmd rag-source-sync
```
