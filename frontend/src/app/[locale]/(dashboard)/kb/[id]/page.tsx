      ...
      <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
        ...
        <aside className="lg:sticky lg:top-6 lg:self-start">
          <div className="border-foreground/10 bg-foreground/[0.02] relative isolate overflow-hidden rounded-2xl border p-6">
            ...
            <dl className="border-foreground/10 mt-6 space-y-2.5 border-t pt-5 text-xs">
              ...
              />
              {pendingCount > 0 && (
                <StatRow label="Pending" value={`${pendingCount} ingesting`} highlight />
              )}
            </dl>
        ...
        <section className="min-w-0">
          ...
          {tab === "documents" ? (
            ...
            <EmptySources hasConnectors={connectors.length > 0} onAdd={() => setWizardOpen(true)} />
          ) : (
            <ul className="border-foreground/10 divide-foreground/8 divide-y rounded-2xl border">
...
function StatRow({
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <dd className={cn("font-mono tabular-nums", highlight ? "text-brand" : "text-foreground")}>
        {value}
      </dd>
    </div>
  );
