...
- Admin: <http://localhost:8080/admin> — `admin@example.com` / `admin123` after `make seed`
- Frontend: <http://localhost:3030> — start with `make dev-frontend` (Docker) or `cd frontend && bun install && bun dev` (local)
...
└── commands/             # Click CLI commands (auto-discovered by `my_ai_agent cmd …`)
...
The generated project ships a Click CLI exposed as `my_ai_agent` (after `make install`):

```bash
my_ai_agent server run --reload          # dev server
my_ai_agent db upgrade                   # apply migrations
my_ai_agent db migrate -m "message"      # create new migration
my_ai_agent user create-admin            # interactive admin creation
my_ai_agent rag-ingest <path> -c docs    # ingest local files
my_ai_agent rag-search "query" -c docs   # semantic search
my_ai_agent rag-collections              # list collections
my_ai_agent celery worker                # start worker
my_ai_agent celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `my_ai_agent --help` for full CLI docs.
...
Using **pgvector** as the vector store with **voyage** embeddings.

```bash
# Ingest local files (recursive)
my_ai_agent rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
my_ai_agent rag-search "your query" --collection documents
```
...
The frontend talks to the backend through Next.js API route handlers in `src/app/api/*` (server-side proxy that forwards auth cookies to the FastAPI backend). Direct calls to `localhost:8080` from the browser are deliberately avoided.
